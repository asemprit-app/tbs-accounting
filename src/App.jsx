...
async function generatePdfBase64FromElement(elementId, filename) {
  const html2pdf = await loadHtml2Pdf();
  const el = document.getElementById(elementId);
  if (!el) throw new Error('Nothing to convert to PDF.');
...
function InvoicePrintModal({ inv, total, onClose, businessName, customers }) {
  ...
  async function handleSendInvoice() {
    const customer = customers.find(c => c.name === inv.client);
    const email = customer?.email || '';
    if (!email) { setSendMsg('No email on file for this customer — add one in the Customers tab first.'); return; }
    setSending(true);
    setSendMsg('');
    try {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}
      ...
      <div style={{ background: '#fff', width: 620, maxHeight: '90vh', overflow: 'auto', borderRadius: 8, padding: 40 }} id="invoice-print-area">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }} className="print-hide">
          <div style={{ fontWeight: 700, fontSize: 18 }}>Invoice {inv.number}</div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {sendMsg && <span style={{ fontSize: 12, color: sendMsg.startsWith('Sent') ? '#0F6E56' : '#B00020', maxWidth: 220 }}>{sendMsg}</span>}
            <button onClick={handleSendInvoice} disabled={sending} style={{ background: '#17365D', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 12px', cursor: sending ? 'default' : 'pointer', fontSize: 13 }}>
